...
class Seagrill : AppCompatActivity() {

    //Declaracion de variables de botones y textos
    var botonvolverseagrill: Button? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_seagrill)

        //Vinculacion de botones y editText
        botonvolverseagrill = findViewById(R.id.botonvolverchiringuito)
        botonvolverseagrill?.setOnClickListener {
            val botonvolverseagrill = Intent(this, MainActivity::class.java)
            startActivity(botonvolverseagrill)
        }
    ...
    fun buscar(view: View) {
        ...
        if (nombre.isEmpty() == false) {
            val fila = baseDatos.rawQuery(
                "select nombre,descripcion,metodo,decoracion from cocteles where restaurante = 'seagrill' and nombre = '$nombre' ",
                null
            )
